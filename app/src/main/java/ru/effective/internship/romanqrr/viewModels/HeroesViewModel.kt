...
class HeroesViewModel : ViewModel() {
    var heroList : List<HeroModel> = listOf()
    fun getHeroList(){
        viewModelScope.launch {
            val lastResult = MarvelAPI.retrofitService.getHeroes(APIKey.apiKey)
            if(lastResult.data?.results == null || lastResult.data?.count == null){
                ...
                heroList = ArrayList(lastResult.data.count)
                lastResult.data.results.forEachIndexed{
                    ...
                    (heroList as ArrayList<HeroModel>)[index] = HeroModel(hero.name?: "", 0, hero.description?:"", imageURL)
                }
            }
        }
    }
